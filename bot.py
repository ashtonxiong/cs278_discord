...
class SpotifyBot:
    def __init__(self, client_id, client_secret, redirect_uri):
        ...
        self.sp_oauth = SpotifyOAuth(client_id=self.client_id, client_secret=self.client_secret, redirect_uri=self.redirect_uri, scope="user-read-playback-state")
        self.user_tokens = {}
    ...
    async def get_fresh_token(self, user_id):
        ...
        if user_id in self.user_tokens:
            ...
            if self.sp_oauth.is_token_expired(token_info):
                ...
                self.user_tokens[user_id] = new_token_info  # Update stored token
    ...
    async def setup_spotify_commands(self, bot):
        ...
        async def authenticate_spotify(interaction: discord.Interaction):
            ...
            self.user_tokens[user_id] = self.sp_oauth.get_authorize_url()
            await interaction.response.send_message(f"Please authenticate with Spotify using this URL: {self.user_tokens[user_id]}")
        ...
        async def playing(interaction: discord.Interaction):
            ...
            if not access_token:
                await interaction.response.send_message("Please authenticate with Spotify first using /authenticate_spotify.")
                return
